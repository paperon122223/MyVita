// ================================================================
// database.ts — Servicio SQLite migrado de Cordova
...
export const DatabaseService = {
   ...
   */
  async createTables(database: DBInstance): Promise<void> {
    const tables = [
      // Usuarios
   ...
   */
  async getDB(): Promise<DBInstance> {
    if (!db) {
      return this.init();
    }
    return db;
  ...
  async marcarAlamaComoTomada(alarmaId: string): Promise<void> {
    ...
    await database.executeSql(
      `UPDATE alarmas SET tomado = 1, updated_at = ? WHERE id = ?`,
      [now, alarmaId]
    );

    // Crear registro en tabla tomas
    const alarma = await database.executeSql(
      `SELECT * FROM alarmas WHERE id = ?`,
      [alarmaId]
    );

    if (alarma[0].rows._array.length > 0) {
      const a = alarma[0].rows._array[0];
      await database.executeSql(
        `INSERT INTO tomas (
  ...
  async marcarComoSincronizado(tabla: string, id: string): Promise<void> {
    ...
    await database.executeSql(
      `UPDATE ${tabla} SET synced_at = ? WHERE id = ?`,
      [now, id]
    );
  },
